...
int main() {
	...
	queue q;
	init_queue(&q); //initialize queue q
	...
	for (int k = 0; k < G.V; k++) { //for each vertex
		if (indegree[k] == 0) { //if the current indegree is 0
			add(q, ((void *)k)); //add it to the rear of the queue
		}
	}

	for (int k = 0; k < G.V; k++) { //for each vertex
		int vertex = ((int)front(q)); //get the first value from the queue
		rear(q); //take that value off the front of the queue
		...
		while (current != NULL) { //while the edge list isnt empty
			...
			if (indegree[current->edge.toVertex] == 0) { //if the indgree is now zero
				add(q, ((void *)current->edge.toVertex)); //add the value to the rear of the queue
			}
